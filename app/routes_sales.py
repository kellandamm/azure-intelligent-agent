...
async def get_top_products(
    ...
    try:
        ...
        with db_conn.get_connection() as conn:
            ...
            # Build RLS filter
            region_filter = ""
            query_params = [start_date, end_date, start_date, end_date, prev_start, start_date, start_date, end_date]
            
            if user_region:
                region_filter = " AND s.Region = ?"
                # Add region parameter for each query section that uses sales data
                query_params.extend([user_region, user_region, user_region, user_region])
